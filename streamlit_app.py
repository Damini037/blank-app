...
def load_data(file_path):
    try:
        # Load the data with chunking to handle large files
        chunk_list = []
        chunksize = 50000
        for chunk in pd.read_csv(file_path, encoding="utf-8", chunksize=chunksize, on_bad_lines="skip"):
...
if data is None or data.empty:
    ...
    # Display the column names to help users debug
    ...
    def busiest_hours():
        if "tpep_pickup_datetime" in data.columns:
            ...
            st.error("Datetime column 'tpep_pickup_datetime' not found in the dataset.")
